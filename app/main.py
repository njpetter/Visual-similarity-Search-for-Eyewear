"""
FastAPI application for Visual Similarity Search
"""
...
async def search_similar(
    ...
    Supports multi-modal search with text modifiers (Bonus Feature)
    """
    try:
        # Save uploaded image
        ...
        # Smart cropping: Detect and crop eyewear region if needed (Bonus Feature)
        ...
        search_results = vector_db.search(query_features, k=20, filters=filters, product_db=db)
        
        # Apply multi-modal search (text modifier) if provided
        if text_modifier:
        ...
        # Get product details
        products = []
        for product_id, similarity_score in boosted_results[:10]:  # Top 10
